...
describe("Sitzung", () => {
  it("zählt mit, ohne etwas zu verbuchen", () => {
    ...
    expect(t).toEqual({ matches: 3, wins: 2, draws: 0, losses: 1, praemien: 2, medals: { gold: 1, silber: 0, bronze: 1 } });
    // Der Fortschritt ist unberührt
    expect(p.siegpraemien).toBe(0);
    expect(p.medaillen).toEqual({ gold: 0, silber: 0, bronze: 0 });
  });
  ...
  it("verbucht beim Verlassen alles auf einmal: eine Prämie pro Sieg", () => {
    ...
    expect(sum.medals).toEqual({ gold: 1, silber: 1, bronze: 0 });
    expect(p.medaillen).toEqual({ gold: 1, silber: 1, bronze: 0 });
    // Erfahrung: 10 + 10 + 2 + 5
